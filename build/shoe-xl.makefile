...
.PHONY: help hello flag_test self_install self_uninstall self_update generate_doc generate_makefile git_hooks gcloud_init kill load_sql psql status
##################################################
# Colors
##################################################
...
hello:
	@printf "${INFO}sh shoe-xl.sh hello${EOL}"
	@sh shoe-xl.sh hello

## Prints flag status
flag_test:
	@printf "${INFO}sh shoe-xl.sh flag_test${EOL}"
	@sh shoe-xl.sh flag_test
...
self_install:
	@printf "${INFO}sh shoe-xl.sh self_install${EOL}"
	@sh shoe-xl.sh self_install

## Uninstall script from system
self_uninstall:
	@printf "${INFO}sh shoe-xl.sh self_uninstall${EOL}"
	@sh shoe-xl.sh self_uninstall

## Update script from @update-link
self_update:
	@printf "${INFO}sh shoe-xl.sh self_update${EOL}"
	@sh shoe-xl.sh self_update
...
generate_doc:
	@printf "${INFO}sh shoe-xl.sh generate_doc${EOL}"
	@sh shoe-xl.sh generate_doc
...
generate_makefile:
	@printf "${INFO}sh shoe-xl.sh generate_makefile${EOL}"
	@sh shoe-xl.sh generate_makefile

## Install git hooks
git_hooks:
	@printf "${INFO}sh shoe-xl.sh git_hooks${EOL}"
	@sh shoe-xl.sh git_hooks

## Configure gcloud
gcloud_init:
	@printf "${INFO}sh shoe-xl.sh gcloud_init${EOL}"
	@sh shoe-xl.sh gcloud_init

## Kill all runnnig containers
kill:
	@printf "${INFO}sh shoe-xl.sh kill${EOL}"
	@sh shoe-xl.sh kill

## Load local sql script
load_sql:
	@printf "${INFO}sh shoe-xl.sh load_sql${EOL}"
	@sh shoe-xl.sh load_sql

## Open postgres terminal in docker
psql:
	@printf "${INFO}sh shoe-xl.sh psql${EOL}"
	@sh shoe-xl.sh psql

## Print docker status
status:
	@printf "${INFO}sh shoe-xl.sh status${EOL}"
	@sh shoe-xl.sh status

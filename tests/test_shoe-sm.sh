...
test_hello_function_should_return_given_option() {
...
test_option_should_fail_validation() {
    assert_fails "$(${src_file} hello --who 123)"
}

test_option_should_fail_validation_with_correct_error_message() {
    assert_equals "error: invalid \"who\", expected \"/^[a-zA-Z-]+$/\", \"123\" given" "$(_remove_colors "$(${src_file} hello --who 123)")"
}

test_hello_function_should_return_given_short_option() {
...
test_short_option_should_fail_validation() {
    assert_fails "$(${src_file} hello --w 123)"
}

test_short_option_should_fail_validation_with_correct_error_message() {
    assert_equals "error: invalid \"who\", expected \"/^[a-zA-Z-]+$/\", \"123\" given" "$(_remove_colors "$(${src_file} hello -w 123)")"
}

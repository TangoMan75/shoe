...
function mp3-update-tag() {
    ...
    find "${source}" -type f -name '*.mp3' | while read -r file_path; do
        ...
        if [ -z "$(echo "${_basename}" | sed -E 's/^[0-9]+\. .+//')" ]; then
            ...
            # update file tag
            _echo_info "id3tool --set-artist \"${artist}\" --set-year \"${year}\" --set-album \"${album}\" --set-track \"${track}\" --set-title \"${title}\" \"${file_path}\"\n"
            id3tool --set-artist "${artist}" --set-year "${year}" --set-album "${album}" --set-track "${track}" --set-title "${title}" "${file_path}"
        ...
        # print undo
        printf 'id3tool --set-artist "%s" --set-year "%s" --set-album "%s" --set-track "%s" --set-title "%s" "%s"\n' "${original_artist}" "${original_year}" "${original_album}" "${original_track}" "${original_title}" "${file_path}" >> "${source}/mp3-update-tag_undo.sh"
    done
}
...
alias dlogs='docker-logs' ## docker-logs alias

## Print container logs (interactive)
function docker-logs() {
    function _usage() {
        _echo_success 'usage:' "$1" "$2"; _echo_primary 'docker-logs (container) -h (help)\n'
    }
    ...
    while [ "$#" -gt 0 ]; do
        ...
        while getopts :h option; do
            case "${option}" in
                h) _echo_warning 'docker-logs\n';
                    _echo_success 'description:' 2 14; _echo_primary 'Print container logs (interactive)\n'
                    _usage 2 14
                    return 0;;
    ...
    _echo_info "docker logs \"${container}\"\n"

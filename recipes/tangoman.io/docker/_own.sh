...
_own() {
    ...
    if [ -d "${_target}" ];then
        _echo_info "chown -R \"${_owner}:${_group}\" \"${_target}\"\n"
        chown -R "${_owner}:${_group}" "${_target}"

        _echo_info "chmod 775 -R \"${_target}\"\n"
        chmod 775 -R "${_target}"
    elif [ -f "${_target}" ];then
        _echo_info "chown \"${_owner}:${_group}\" \"${_target}\"\n"
        chown "${_owner}:${_group}" "${_target}"

        _echo_info "chmod 664 -R \"${_target}\"\n"
        chmod 664 -R "${_target}"
    else
        _echo_warning "file not found \"${_target}\"\n"
    fi
}

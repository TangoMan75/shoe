...
build() {
    ...
    if [ "${_database_type}" = sqlite ]; then
        if [ "${env}" = prod ]; then
            _echo_info 'docker compose -f compose.prod.yml build\n'
            docker compose -f compose.prod.yml build
        elif [ "${env}" = dev ]; then
            _echo_info 'docker compose -f compose.dev.yml build\n'
            docker compose -f compose.dev.yml build
        else
            _echo_info 'docker compose build\n'
    ...
    if [ "${_database_type}" = postgresql ]; then
        if [ "${env}" = prod ]; then
            _echo_info 'docker compose -f compose.postgres.prod.yml build\n'
            docker compose -f compose.postgres.prod.yml build
        elif [ "${env}" = dev ]; then
            _echo_info 'docker compose -f compose.postgres.dev.yml build\n'
            docker compose -f compose.postgres.dev.yml build
        else
            _echo_info 'docker compose -f compose.postgres.yml build\n'

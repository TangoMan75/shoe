#!/bin/bash

## Check if a git cherry-pick operation is in progress
##
## {
##   "namespace": "git",
##   "summary": "Check if a git cherry-pick operation is currently in progress.",
##   "requires": [
##     "git",
##     "command"
##   ],
##   "depends": [
##     "_echo_success",
##     "_echo_primary",
##     "_echo_warning",
##     "_echo_danger",
##     "_echo_info"
##   ],
##   "parameters": [
##     {
##       "position": 1,
##       "name": "arguments",
##       "type": "str",
##       "description": "Optional flags (-v: verbose, -h: help)",
##       "default": null,
##       "nullable": true
##     }
##   ]
## }
function _is_cherry_pick_in_progress() {
    function _usage() {
        _echo_success 'usage:' "$1" "$2"; _echo_primary '_is_cherry-pick_in_progress -v (verbose) -h (help)\n'
    }

    #--------------------------------------------------
    # Variables
    #--------------------------------------------------

    local verbose=false

    #--------------------------------------------------
    # Parse options
    #--------------------------------------------------

    local option
    while getopts :vh option; do
        case "${option}" in
            v) verbose=true;;
            h) _echo_warning '_is_cherry_pick_in_progress\n';
                _echo_success 'description:' 2 14; _echo_primary 'Check cherry-pick is in progress\n'
                _usage 2 14
                return 0;;
            \?) _echo_danger "error: invalid option \"${OPTARG}\"\n"
                return 1;;
        esac
    done

    #--------------------------------------------------
    # Check git installation
    #--------------------------------------------------

    if [ ! -x "$(command -v git)" ]; then
        _echo_danger 'error: git required, enter: "sudo apt-get install -y git" to install\n'
        return 1
    fi

    #--------------------------------------------------
    # Check git directory
    #--------------------------------------------------

    if [ -z "$(git rev-parse --show-toplevel 2>/dev/null)" ]; then
        _echo_danger 'error: Not a git repository (or any of the parent directories)\n'
        return 1
    fi

    #--------------------------------------------------

    if [ "${verbose}" = true ]; then
        _echo_info "test -f \"$(git rev-parse --git-dir)/CHERRY_PICK_HEAD\"\n"
    fi

    if [ -f "$(git rev-parse --git-dir)/CHERRY_PICK_HEAD" ]; then
        echo true
        return 0
    fi

    echo false
}
